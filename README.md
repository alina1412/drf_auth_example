...
```
- check docs: http://127.0.0.1:8000/swagger/
...
5. Изменения как обновление информации о любом пользователе или его роли может выполнять только админ, изменять информацию "о себе" кроме роли может каждый зарегистрированный (кто не гость).
6. После изменения роли нужно перелогиниться (обновить токен).
7. После удаления (деактивации) пользователя его токен становится недействительным.
8. В базе минимальное количество таблиц (MVP): user, role, category, recipe. Рецепты и их категории - условные эндпоинты для сайта.
Таблицы связаны: в каждой категории может быть много рецептов. Только список рецептов доступен гостю.
9. Для запроса зарегистрированного пользователя нужно передать -H 'Authorization:bearer xxx' в хедере (или можно через соответствующую функцию в сваггере).
